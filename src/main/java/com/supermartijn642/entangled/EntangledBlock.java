...
public class EntangledBlock extends BaseBlock {
    ...
    public EntangledBlock(){
        super("block", true, Properties.of(new Material.Builder(MaterialColor.COLOR_BROWN).noCollider().build()).harvestTool(ToolType.PICKAXE).sound(SoundType.STONE).strength(2f));
        this.registerDefaultState(this.defaultBlockState().setValue(ON, false));
    }

    @Override
    public boolean use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult result){
        if(worldIn.isClientSide)
            return true;
        ItemStack stack = playerIn.getItemInHand(hand);
        if(playerIn.isSneaking() && stack.isEmpty() && state.getValue(ON)){

    ...
    public void appendBody(List<ITextComponent> tooltip, IDataAccessor accessor, IPluginConfig config){
        ...
        if(tile instanceof EntangledBlockEntity){
            if(((EntangledBlockEntity)tile).isBound()){
                ...
                if(((EntangledBlockEntity)tile).getBoundDimension() == accessor.getWorld().dimension())
                    ...
                    ITextComponent dimension = TextComponents.dimension(((EntangledBlockEntity)tile).getBoundDimension()).color(TextFormatting.GOLD).get();

package com.supermartijn642.entangled.generators;

import com.supermartijn642.core.generator.RecipeGenerator;
import com.supermartijn642.core.generator.ResourceCache;
import com.supermartijn642.entangled.Entangled;
import net.minecraft.init.Items;
import net.minecraft.item.Item;

/**
 * Created 01/09/2022 by SuperMartijn642
 */
public class EntangledRecipeGenerator extends RecipeGenerator {

    public EntangledRecipeGenerator(ResourceCache cache){
        super("entangled", cache);
    }

    @Override
    public void generate(){
        // Entangled block
        this.shaped(Item.getItemFromBlock(Entangled.block))
            .pattern("ABA")
            .pattern("BCB")
            .pattern("ABA")
            .input('A', Items.ENDER_PEARL)
            .input('B', "obsidian")
            .input('C', "chestWood")
            .unlockedBy(Items.ENDER_PEARL);
        this.shapeless("entangled_clear_nbt", Item.getItemFromBlock(Entangled.block))
            .input(Item.getItemFromBlock(Entangled.block))
            .noAdvancement();

        // Entangled Binder
        this.shaped(Entangled.item)
            .pattern(" AB")
            .pattern(" CA")
            .pattern("C  ")
            .input('A', Items.ENDER_PEARL)
            .input('B', "gemDiamond")
            .input('C', "obsidian")
            .unlockedBy(Items.ENDER_PEARL);
    }
}
